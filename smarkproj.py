...
def process_data(df, scaler=StandardScaler):
    ...
    '''
    out = df.replace({"low_bike_demand": 0, "high_bike_demand": 1}, inplace=True)
    for column in out.columns:
        out[column] = scaler.fit_transform(out[column])
    return out

fil = remove_constant_data(raw)
fil = process_data(fil)


def plot_hourly_avg(dataframe):
    hourly_avg = dataframe.groupby("hour_of_day")[["temp","dew","humidity","precip","snow","snowdepth","windspeed","cloudcover","visibility","increase_stock"]].mean().reset_index()
    ...
    plt.plot(hourly_avg["hour_of_day"], hourly_avg["snow"], label="Snow")
...
def plot_weekday_avg():
    weekday_avg = fil.groupby("day_of_week")[["temp","dew","humidity","precip","snow","snowdepth","windspeed","cloudcover","visibility"]].mean().reset_index()
    plt.figure(figsize=(12, 8))
    ...
    plt.plot(weekday_avg["day_of_week"], weekday_avg["snow"], label="Snow")
...
def plot_monthly_avg():
    monthly_avg = fil.groupby("month")[["temp","dew","humidity","precip","snow","snowdepth","windspeed","cloudcover","visibility"]].mean().reset_index()
    plt.figure(figsize=(12, 8))
    ...
    plt.plot(monthly_avg["month"], monthly_avg["snow"], label="Snow")
